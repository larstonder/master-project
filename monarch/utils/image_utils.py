...
def save_rgb_images_to_video(
    sensor_outputs, output_path, fps=20, gamma=2.2, brightness=1.5, apply_gamma=True
):
    """
    ...
    print(
        f"Video settings: brightness={brightness}, gamma={gamma if apply_gamma else 'disabled'}"
    )
    ...
    for i, sensor_output in enumerate(sensor_outputs):
        ...
        if i % 10 == 0:
            print(
                f"Frame {i}: shape={rgb.shape}, dtype={rgb.dtype}, min={rgb.min():.4f}, max={rgb.max():.4f}"
            )
    ...
    writer.close()
    print(
        f"Video saved to {output_path} with {len(sensor_outputs)} frames at {fps} fps"
    )


def save_rgb_image(sensor_output, output_path, brightness=1.5, gamma=2.2, apply_gamma=True):
    """
    Save an RGB image to a file.
    """
    # base the formatting on the save_rgb_images_to_video function
    # but only save one frame to a .png file
    
    rgb = sensor_output.rgb_image

    # Check if RGB data exists
    if rgb is None:
        print(f"No RGB data in sensor output, exiting.")
        return
